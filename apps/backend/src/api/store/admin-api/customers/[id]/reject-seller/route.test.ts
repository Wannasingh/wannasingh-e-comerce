...
describe("reject-seller API route POST", () => {
  let mockReq: {
    params: { id: string };
  ...
  it("should reject the seller and return 200 with updated customer info", async () => {
    mockCustomerService.retrieveCustomer.mockImplementation((id: string) => {
      if (id === "cust_caller")
        return Promise.resolve({ id: "cust_caller", email: "wannasingh.khan@gmail.com" });
      if (id === "cust_target")
    ...
    mockCustomerService.updateCustomers.mockResolvedValue({
      id: "cust_target",
      metadata: { seller_requested: "false", seller_approved: "false" },
    });
